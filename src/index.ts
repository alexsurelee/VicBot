...
export const MAX_EMBED = 2000; // maximum characters allowed per embedded message
let prefix = process.env.PREFIX;
let token = process.env.TOKEN;
let examDataUrl = process.env.EXAM_DATA_URL;
let examDataFile = process.env.EXAM_DATA_FILE;
let examDataUpdate: number = (process.env.EXAM_DATA_UPDATE as unknown) as number;
...
const { forbiddenRanks, socialRanks, adminRank, username, logChannelName } = require('./config.json');
let oneCategory;
let twoCategory;
let threeCategory;
let fourCategory;
...
client.on('guildMemberAdd', async member => {
    ...
    const logChannel: TextChannel = <TextChannel>(
        member.guild.channels
            ...
            .find(channel => channel.name === logChannelName)
    );
...
exports.rank = async function(message, rank) {
    ...
    if (/^[a-zA-Z]{4}[1-4]\d\d$/.test(rank)) rank = rank.slice(0, 4) + '-' + rank.slice(4, 7);

    if (message.guild.roles.find(role => role.name === rank) == null) rank = rank.toLowerCase();
...
exports.organise = async function(message) {
    const isClass = word =>
        word.charAt(5) === '1' || word.charAt(5) === '2' || word.charAt(5) === '3' || word.charAt(5) === '4';
...
client.on('messageReactionAdd', async reaction => {
    if (reaction.emoji.name === '📌') if (reaction.count >= 3 && !reaction.message.pinned) await reaction.message.pin();
});
...
exports.deleteRank = async function(message, args) {
    ...
    if (message.guild.channels.find(channel => channel.name === args[0]) != null)
        await message.guild.channels.find(channel => channel.name === args[0]).delete();
    return;
};
...
exports.getCourse = async function(code, i, j, k, message) {
    ...
    https
        .get(`https://www.victoria.ac.nz/_service/courses/2.1/courses/${name}?year=${currentYear}`, resp => {
            let data = '';
 ...
 * @return {Date}
 */
exports.getExamLastUpdated = function(): number {
    const stats = fs.statSync(examDataFile);
    return stats.mtime.getMilliseconds();
};
...
exports.fetchData = function() {
    ...
    download(examDataUrl, options, function(error) {
        if (error == 404) console.error('URL for exam data returned a 404. Check the URL is valid and working.');
    });
};
...
exports.formatExams = function(message, exams, displayErrors) {
    ...
    for (let i = 0; i < exams.length; i++) {
        ...
        if (exam != undefined) {
            ...
            if (datum != undefined) {
                examDataOutput += `${exam}\t${parseDuration(datum.duration)}\t${parseDate(datum.date)}\t${parseStart(
                    datum.start
                )}\t${parseRooms(datum.rooms)}\n`;
    ...
    for (let i = 0; i < exams.length; i++) {
        ...
        if (exam != undefined) {
            ...
            if (datum != undefined) {
                ...
                const channel = message.guild.channels.find(channel => channel.name == examChannel);
...
exports.examDataEmbed = function(examDataOutput) {
    const embeddedMessage = new MessageEmbed()
        ...
        .addField(
            `Last updated: ${formatTime(new Date().getTime() - module.exports.getExamLastUpdated().getTime())} ago.`,
...
exports.updateConfigUrl = function(url) {
    if (validExamURL(url)) {
        ...
        fs.writeFile('./config.json', JSON.stringify(file, null, 2), function(error) {
            if (error) {
                console.log(error);
                return false;
            }
