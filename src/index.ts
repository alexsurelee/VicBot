...
import * as download from 'download-file'; // fetching exam data
import * as xlsx from 'xlsx'; // exam data speadsheet
...
let examDataUpdate: number = +process.env.EXAM_DATA_UPDATE;
const examData = {};
...
exports.rank = async function(message, rank) {
  ...
  if (noDashCourseRegex.test(rank)) rank = `${rank.slice(0, 4)}-${rank.slice(4, 7)}`;
  ...
  if (message.guild.roles.find((role) => role.name.toUpperCase() === rank.toUpperCase()) == null) {
  ...
  if (
    ...
    if (!aliasRegex.test(rank)) {
      ...
      return message.reply(`Removed you from ${rankChannel || rank} successfully.`);
    }
...
exports.organise = async function(message) {
  ...
  for (let i = 0; i < threeLength; i++) { if (
      ...
      (resp) => {
        ...
        resp.on("end", () => {
          JSON.parse(data, (key, value) => {
            if (key === "title") { message.guild.channels
...
exports.deleteRank = async function(message, args) {
  ...
  if (message.guild.channels.find((channel) => channel.name === args[0]) != null) { await message.guild.channels
      .find((channel) => channel.name === args[0])
      .delete(); }
};
...
exports.fetchData = function() {
  ...
  download(examDataUrl, options, (error) => {
    if (error == 404) { console.error(
...
exports.formatExams = function(message, exams, displayErrors) {
  ...
  for (let i = 0; i < exams.length; i++) {
    ...
    if (exam != undefined) {
      ...
      else if (displayErrors) { message.reply(
...
exports.updateConfigUrl = function(url) {
  if (validExamURL(url)) {
    ...
    fs.writeFile(CONFIG, JSON.stringify(file, null, 2), (
      error,
    ) => {
      if (error) {
        console.log(error);
