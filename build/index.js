...
const BOT_CONFIG = __dirname + "/../botConfig.json";
const CONFIG = __dirname + "/../config.json";
const wrappers = require(__dirname + "/../data/wrappers.json");
...
try {
    if (fs.existsSync(BOT_CONFIG)) {
        const { PREFIX, TOKEN } = require(BOT_CONFIG);
        if (PREFIX) {
            prefix = PREFIX;
        }
        if (TOKEN) {
...
try {
    if (fs.existsSync(CONFIG)) {
        const { EXAM_DATA_URL, EXAM_DATA_FILE, EXAM_DATA_UPDATE } = require(CONFIG);
        if (!examDataUrl) {
...
const { socialRanks, adminRank, username, logChannel } = require(CONFIG);
let oneCategory;
let twoCategory;
let threeCategory;
let fourCategory;
 ...
 */
exports.rank = function (message, rank) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (wrappers[rank]) {
            if (!message.guild.roles
                ...
                message.channel.send(`${rank} is a wrapper course! You'll be added to the channel that this course wraps.`);
            }
            rank = wrappers[rank];
        }
        if (!socialRanks.includes(rank.toLowerCase()) && !courseRegex.test(rank)) {
        ...
        else if (!message.guild.roles
            .find(role => role.name.toUpperCase() === rank.toUpperCase())
            .members.has(message.author.id)) {
            yield message.member.roles.add(message.guild.roles.find(role => role.name.toUpperCase() === rank.toUpperCase()));
            if (!aliasRegex.test(rank) && !socialRanks.includes(rank)) {
                const rankChannel = message.guild.channels.find(channel => channel.name.toUpperCase() === rank.toUpperCase());
            ...
            yield message.member.roles.remove(message.guild.roles.find(role => role.name.toUpperCase() === rank.toUpperCase()));
 ...
 * @param {Discord.Message} message the message sent
 */
exports.log = function (commandName, message) {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 * @param {Discord.Message} message the message sent
 */
exports.organise = function (message) {
    return __awaiter(this, void 0, void 0, function* () {
        const isClass = word => word.charAt(5) === "1" ||
            word.charAt(5) === "2" ||
            word.charAt(5) === "3" ||
            word.charAt(5) === "4";
        ...
        yield oneNameArray.sort();
        yield twoNameArray.sort();
        yield threeNameArray.sort();
        yield fourNameArray.sort();
        // if the channel is not in the same location as its sorted array location, move it.
        for (let i = 0; i < oneLength; i++)
            if (message.guild.channels.find(channel => channel.name === oneNameArray[i])
                .position != i)
                yield message.guild.channels
                    .find(channel => channel.name === oneNameArray[i])
                    .setPosition(i);
        for (let i = 0; i < twoLength; i++)
            if (message.guild.channels.find(channel => channel.name === twoNameArray[i])
                .position != i)
                yield message.guild.channels
                    .find(channel => channel.name === twoNameArray[i])
                    .setPosition(i);
        for (let i = 0; i < threeLength; i++)
            if (message.guild.channels.find(channel => channel.name === threeNameArray[i])
                .position != i)
                yield message.guild.channels
                    .find(channel => channel.name === threeNameArray[i])
                    .setPosition(i);
        for (let i = 0; i < fourLength; i++)
            if (message.guild.channels.find(channel => channel.name === fourNameArray[i])
                .position != i)
                yield message.guild.channels
 ...
 */
exports.newRank = function (message, args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let levelParent;
        if (args[0].charAt(5) === "1")
            levelParent = oneCategory;
        else if (args[0].charAt(5) === "2")
            levelParent = twoCategory;
        else if (args[0].charAt(5) === "3")
            levelParent = threeCategory;
        else if (args[0].charAt(5) === "4")
            levelParent = fourCategory;
        yield message.guild.channels.create(args[0], {
            type: "text",
        ...
        yield this.organise(message);
        // pull the course title and set the topic
        const name = args[0].slice(0, 4) + args[0].slice(5, args[0].length);
        const currentYear = new Date().getFullYear();
        const https = require("https");
        https
            .get(`https://www.wgtn.ac.nz/_service/courses/2.1/courses/${name}?year=${currentYear}`, resp => {
            let data = "";
 ...
 * @param {string[]} args array of strings in the message
 */
exports.deleteRank = function (message, args) {
    return __awaiter(this, void 0, void 0, function* () {
        if (message.guild.roles.find(role => role.name === args[0]) != null)
...
exports.isPaper = function (channel) {
    ...
    if (channel.type !== "text")
        return false;
    return (channel.parent.name === "100-level" ||
 ...
 */
exports.getCourse = function (code, i, j, k, message) {
    return __awaiter(this, void 0, void 0, function* () {
        const https = require("https");
            ...
            resp.on("end", () => {
                JSON.parse(data, function (key, value) {
                    if (key === "id" && value && /^[a-zA-Z]{4}[1-4]\d\d$/.test(value)) {
                        ...
                        if (!message.guild.channels.find(channel => channel.name === hyphenatedName))
...
function checkExamUpdates() {
    if (new Date().getMilliseconds() - module.exports.getExamLastUpdated() > examDataUpdate) {
        module.exports.fetchData();
        if (new Date().getMilliseconds() - module.exports.getExamLastUpdated() < 10000) {
...
exports.fetchData = function () {
    ...
    download(examDataUrl, options, function (error) {
        if (error == 404)
            console.error("URL for exam data returned a 404. Check the URL is valid and working.");
    });
};
/**
    ...
    for (let i = 0; i < exams.length; i++) {
        ...
        if (exam != undefined) {
            ...
            if (datum != undefined) {
                ...
                message.reply(`couldn't find exam data for '${exams[i]}'. Does the course exist for the current trimister?`);
        }
        else if (displayErrors)
    ...
    for (let i = 0; i < exams.length; i++) {
        ...
        if (exam != undefined) {
            ...
            if (datum != undefined) {
                ...
                const channel = client.channels.find(channel => channel.name == examChannel);
                if (channel != undefined) {
                    // channel exists for the exam
                    const examDataOutput = module.exports.formatExams(message, [exam], false); // get the formatted data
                    ...
                    message.reply(`couldn't find the channel for '${exams[i]}'. Does the channel #${examChannel} exist?`);
            }
            else if (displayErrors)
                message.reply(`couldn't find exam data for '${exams[i]}'. Does the course exist for the current trimister?`);
        }
...
exports.updateConfigUrl = function (url) {
    if (validExamURL(url)) {
        ...
        fs.writeFile(CONFIG, JSON.stringify(file, null, 2), function (error) {
            if (error) {
                console.log(error);
                return false;
            }
